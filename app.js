...
app.get("/2fa/hpi-sss/value", (req, res) => {
  res.send(authenticator.generateToken(process.env.HPI_SSS_2FA_KEY));
});
...
app.get("/2fa/hpi-sss", (_req, res) => {
  const initialToken = authenticator.generateToken(process.env.HPI_SSS_2FA_KEY);
  res.send(`
    <!DOCTYPE html>
    <html>
    <head>
      <meta charset="utf-8">
      <title>HPI SSS 2FA Token</title>
      <style>
        .notification {
          color: green;
          margin-left: 10px;
